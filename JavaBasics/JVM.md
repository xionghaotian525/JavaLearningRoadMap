...
- 在内存中生成这个类的java.lang.Class对象，作为**方法区**这个类的各种数据的访问入口。
...
注：**类的加载过程（机制）**加上“使用”和“卸载”可以看做**类的生命周期**
...
![](/Res/images/JVM堆-堆的永久代-元空间.jpg)
     1. 永久代是 HotSpot 虚拟机对虚拟机规范中方法区的⼀种实现⽅式。
   ...
   2.2 **方法区**
- Java 虚拟机规范把方法区描述为堆的⼀个逻辑部分，它⽤于存储已被虚拟机加载的**类信息、常量、静态变量、即时编译器编译后的代码**等数据。
...
- 线程共享
  - **堆**
   是Java虚拟机所管理的内存中最大的一块。Java堆是被所有线程共享的一块内存区域，在虚拟机启动时创建。此内存区域的唯一目的就是存放**对象实例**，几乎所有的对象实例都在这里分配内存。 
  - **方法区**
   方法区（Method Area）与Java堆一样，是各个线程共享的内存区域，它用于存储已被虚拟机**加载的类信息**、**常量**、**静态变量**、**即时编译器编译后的代码**等数据。 
- 线程私有：
  - **程序计数器**
  程序计数器（Program Counter Register）是一块较小的内存空间，它的作用可以看做是**当前线程所执行的字节码的行号指示器**。 
  - **虚拟机栈**
   与程序计数器一样，Java虚拟机栈（Java Virtual Machine Stacks）也是线程私有的，它的生命周期与线程相同。虚拟机栈描述的是Java方法执行的内存模型：**每个方法被执行的时候都会同时创建一个栈帧（Stack Frame）** 用于存储局部变量表、操作栈、动态链接、方法出口等信息。**每一个方法被调用直至执行完成的过程，就对应着一个栈帧在虚拟机栈中从入栈到出栈的过程**。
  - **本地方法区**
   本地方法栈（Native Method Stacks）与虚拟机栈所发挥的作用是非常相似的，其区别不过是**虚拟机栈为虚拟机执行Java方法（也就是字节码）服务**，而**本地方法栈则是为虚拟机使用到的Native方法服务**。
- 直接内存
### (6).说说对象分配规则
...
![](/Res/images/JVM类加载机制.png)
1. **加载（Loading）：**
JVM通过类加载器（ClassLoader）读取二进制数据（.class文件）。检查文件格式，确认是否为有效的编译后的Java类文件。
1. **链接（Linking）：**
...
1. **类加载检查**：JVM首先检查该类是否已经被加载、链接、初始化。如果没有，则先执行这些步骤。
2. **分配内存**：在堆上为新对象分配内存。内存的大小由对象的类确定（所有属性的大小加上一些额外的空间，用于存储对象头和引用）。
3. **初始化零值**：分配的内存空间在使用前首先会被清零，即所有的对象属性会被设置为默认值（0、false或null）。
4. **设置对象头**：JVM会在对象的内存中写入对象头信息，包括这个对象所属类的信息、对象的Hash码、对象的GC分代年龄等。
5. **执行`<init>`方法**：执行构造方法，这个过程包括父类构造方法的执行（如果有），以及自定义的初始化代码。在执行构造函数之后，对象才被认为是完全构造出来的。

完成这些步骤之后，JVM返回对象的引用，从而可以通过这个引用操作对象
### (9).知道类的生命周期吗？
...
### (10).简述Java的对象结构
在Java中，对象的结构是在堆内存中分配的，在内部由以下几个部分组成：
   1. **对象头**（Header）:
      - 标记字段（Mark Word）：包含对象自身的运行时数据，如哈希码（HashCode）、垃圾收集信息、锁状态标志、线程持有的锁等。
      - 类型指针（Type Pointer）：指向它的类元数据的指针，用于确定对象是哪个类的实例。
   2. **实例数据**（Instance Data）:
      - 对象真正存储的有效信息，包含对象中所有的实例字段，无论是来自当前类还是父类继承而来的字段。
   3. **对齐填充**（Padding）:
      - 由于JVM要求对象的大小必须是8字节的整数倍，所以可能需要加入0-7字节的对齐填充来满足这一要求。

### (11).如何判断对象可以被回收？
1. **引用计数法**
   给对象中添加⼀个引用计数器，每当有一个地方引用它，计数器就加1；当引用失效，计数器就减1；任何时候计数器为0的对象就是不可能再被使用的。此方法简单，**无法解决对象相互循环引用的问题。**
2. **可达性分析算法**
   ...
   Java 虚拟机使用该算法来判断对象是否可被回收，在 Java 中 `GC Roots` 一般包含以下内容:
      ...
      - 方法区中的常量引用的对象
2. 其他
   - **Finalization**: 一旦垃圾收集器确定没有引用指向一个对象，将调用该对象的finalize()方法（如果这个方法在对象中被重写，且之前未被调用过），给对象一个清理资源的机会。然后在下一次垃圾回收中，如果对象依旧没有被引用，那么它将被回收。
   - **软/弱/虚引用** (Soft/Weak/Phantom References): 这些都是Java的引用类型，垃圾收集器对这些引用类型也有特殊的处理方式。
      - 软引用 (SoftReference)的对象，在系统要发生内存溢出之前，会把这些对象列入回收范围进行第二次回收。如果还是没有足够的内存，才会抛出内存溢出异常。
      - 弱引用 (WeakReference)的对象，只能生存到下一次垃圾回收之前。当垃圾回收器工作时，无论当前内存是否足够，都会回收掉只被弱引用关联的对象。
      - 虚引用 (PhantomReference)的对象，任何时候都可能被垃圾回收器回收，设置虚引用的唯一目的就是在这个对象被回收时收到一个系统通知。 
### (12).JVM的永久代中会发生垃圾回收么？
...
然而，从Java 8开始，永久代被元空间（Metaspace）所取代。元空间使用本地内存来存储类的元数据，其默认大小只受本地内存限制。在元空间中，与永久代相似，垃圾回收的行为主要包括**类的卸载**和**废弃的即时编译代码的回收**。由于元空间与直接内存相似，因此其大小不再由Java堆的大小直接限制，避免了永久代过小导致的`OutOfMemoryError`。
...
现在的商业虚拟机都采用这种收集算法来回收**新生代**，但是并不是将新生代划分为大小相等的两块，而是**分为一块较大的 Eden 空间和两块较小的 Survivor 空间**，每次使用 Eden 空间和其中一块 Survivor。在回收时，将 Eden 和 Survivor 中还存活着的对象一次性复制到另一块 Survivor 空间上，最后清理 Eden 和使用过的那一块 Survivor。
...
### (14).JDK自带监控和故障处理工具有哪些？
oracle JDK监控和故障处理命令有
- jps - JVM Process Status Tool：显示当前系统中的所有Java进程，可以用来找到Java进程的进程ID。
- jstat - JVM Statistics Monitoring Tool：用于监控JVM内存和垃圾收集的统计信息。
- jinfo - Configuration Info for Java：可以显示和调整运行中的Java进程的配置信息。
- jmap - Memory Map for Java：生成JVM的内存映射（heap dump），用于分析堆使用情况。
- jhat - JVM Heap Dump Browser：与jmap一起使用，分析heap dump文件。
- jstack - Stack Trace for Java：用于生成Java进程的线程快照，分析线程的堆栈信息，非常有用于分析死锁。
- VisualVM：提供一个GUI界面，整合了多个JDK命令行工具的功能，可以监控、故障处理、分析内存泄漏、配置和监控垃圾收集器等。
- jcmd - Send Diagnostic Command to JVM：可以发送不同的诊断命令到JVM，它整合了多个上述工具的功能。
- hs_err_pid.log：这不是一个命令，而是当JVM发生崩溃时自动生成的错误日志文件，它包含了故障时的详细信息，对于分析和解决故障非常有用。
- Java Mission Control: 这是一个更高级的监控工具集，它包含了JMX Console和Java Flight Recorder。
...
1. Minor GC发生在年轻代（Young Generation）的内存满时。在Java堆内存管理中，年轻代主要包括了Eden区和两个Survivor区（通常被称为S0和S1）。当应用程序创建新对象时，这些对象首先被分配到Eden区。**当Eden区满了，就会触发一次Minor GC**，此过程通常包括：
   - 标记可达对象。
   - 清除不可达对象，并将可达对象移动到一个Survivor区。
   - 对象在Survivor区之间来回复制，每次复制时年龄增加，当达到一定年龄阈值后，移动到老年代。

2. Full GC涉及整个Java堆内存，包括年轻代、老年代（Old Generation）以及方法区（如果使用了永久代或元空间的话）。Full GC发生的情况包括但不限于：
   1. **调用 System.gc()**
   2. **老年代空间不足**
   3. **空间分配担保失败**
   4. **JDK 1.7 及以前的永久代空间不足**
...
### (5).逃逸分析
### (6).移除永久代的原因
### (7).Class.forName()和ClassLoader.loadClass()的区别
