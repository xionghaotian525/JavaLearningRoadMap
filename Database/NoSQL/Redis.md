...
Redis提供了两种不同的持久化方法来将数据存储到硬盘里面。一种方法叫**快照 （snapshotting）**，它可以将存在于某一时刻的所有数据都写入硬盘里面。另一种方法叫**只追加文件 （append-only file，AOF）**，它会在执行写命令时，将被执行的写命令复制到硬盘里面。可以单独使用其中一种或将二者结合使用。
